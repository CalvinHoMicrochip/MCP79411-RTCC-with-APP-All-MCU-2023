...
void OLED_Init(void)
{
    ...
    DELAY_milliseconds(100); // MCC Delay added by Calvin - Must import DELAY foundation service 
...
void  OLEDWrCmd(uint8_t command)
{
       ...
       TxBuffer[1] = command;
        I2C0_Host.Write(OLED_ADDRESS, TxBuffer, 2);
        while (I2C0_Host.IsBusy()) ;    
    
//       i2c_write1ByteRegister(OLED_ADDRESS, OLED_Command_Mode, command);
//       i2c_writeNBytes(OLED_ADDRESS, TxBuffer, 2) ;       
//       I2C2_SetBuffer(TxBuffer, 2) ;
//       I2C2_MasterWrite();  
//       while(I2C2_Status.state == I2C2_IDLE);
}
...
//       i2c_writeNBytes(OLED_ADDRESS, TxBuffer, 2) ;
//       I2C_MasterWrite(TxBuffer, 2, OLED_ADDRESS, &I2C_status);  
